...
fluxpath <- file.path(datpath, "raw", "AmeriFlux/")
ghcndpath <- file.path(datpath, "raw", "GHCNd/")
npnpath <- file.path(datpath, "raw/NPN/") #ETI Gauge from npn website
# note you must manually download the PRECIP_CO90_DAILY.csv file from
# here https://nadp.slh.wisc.edu/networks/nadp-precipitation-network/
# select daily, then CO90 on the righthand side of the page to download
# the csv

# Toggle for plot display
view_plots <- FALSE # If you just need to rerun quickly, FALSE will skip plotting
...
# Replace with your own ameriflux creds...
ameriflux <- fetchAmeriFlux(fluxpath, ameriflux_user_id = 'MilesMooreNWT',
                            ameriflux_email = 'Miles.Moore-1@colorado.edu')
ghcnd <- fetchGHCND(ghcndpath)
npn <- readr::read_csv(list.files(npnpath, full.names = T),
                       # first line is metadata
                       skip =1) |> 
  dplyr::mutate(siteID = "CO90")
...
loggerTemp_out <- lapply(nwtlogger, function(x) { 
  subset(x, select = grep("site|logger|date|^airtemp|^flag_airtemp", 
                          names(x), ignore.case = T)) } )
...
clean_npn <- npn |> 
  rename(
    logger = siteID,
    measurement = qualifiedPrecipInches,
    date_utc = dateTimeGMT) |> 
  mutate(
    date = lubridate::ymd(date_utc),
    logger = paste0(logger, "_ETI"),
