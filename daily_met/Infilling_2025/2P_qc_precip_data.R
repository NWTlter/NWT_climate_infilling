...
chartppt_out$measurement[!(is.na(chartppt_out$flag_ppt_tot)|chartppt_out$flag_ppt_tot == 'n')] <- NA
...
# -- 3. C1 loggers & NPN (ETI Gauge) -----

# Should be ready to go as well, MM recommends visually inspecting each year.
c1logppt_out <- c1logppt |> filter(flag_ppt_tot == 'n') |> 
  dplyr::mutate(
    station_id = dplyr::case_when(
      logger ==  "cr23x" ~ paste(local_site, "logger"),
      logger ==  "cr1000" ~ paste(local_site, "logger"),
      logger ==  "CO90_ETI" ~ "NPN eti",
      TRUE ~ "ERROR"), # otherwise something unexpected
    station_name = local_site,
    latitude = 40.03596085439916,
    longitude = -105.54415060302942,
    elevation = 3022
  )
  
c1logppt_out |> 
  ggplot()+
  geom_point(aes(date, measurement))+
  facet_wrap(~logger)
...
# this is the eti_info, but if it's marked as exactly the same
# it will be ignored in the infilling
# eti_info <- data.frame(rbind(
#   c("NPN", 40.03596085439916, -105.54415060302942, 3022) #C1 ETI Gauge
# ))

# add a tiny bit
eti_info <- data.frame(rbind(
  c("NPN", 40.03596086, -105.54415061, 3023) #C1 ETI Gauge
))


names(eti_info) <- c("local_site", "latitude", "longitude", "elevation")
eti_info[,2:4] <- apply(eti_info[,2:4], 2, as.numeric)
str(eti_info)
eti_info$station_id <- paste(eti_info$local_site, "eti")
eti_info$station_name <- eti_info$local_site
...
allsites <- rbind(ghcnd_sites, ameriflux_sites, snotel_sites) %>%
  rbind(nwt_info[names(.)]) %>% rbind(eti_info[names(.)])
...
# cor.test(~ d1 + USC00050183, dat = test) # allenspark near estes
...
saveRDS(c1logppt_out, paste0(datpath, "/qc/c1loggerPPT_qc.rds")) 
