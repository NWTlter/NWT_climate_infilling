...
sdl_temp <- readRDS(paste0(datpath,"homogenize/nwt_sdl_homogenized_temperature_draft_2023.rds"))
...
summary(duplicated(sdl_temp_pretty$date))
...
#Clip to only include 2018 - 2022
sdl_ppt_pretty <- sdl_ppt_pretty |> dplyr::filter(year > 2017)
# make NA NaN instead (tell sce NaN in this case = no value, not data are missing)
# write out csv with utf-8 and \r\n eol (end of line)
# (need to change this in code settings, check it looks good after write out)
