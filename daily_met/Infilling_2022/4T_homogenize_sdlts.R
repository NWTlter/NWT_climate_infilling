...
#' 
#TODO Clean up comments, incorrect & from CW old code.
...
glimpse(dat_out)
#TODO Pick up from here, just need to append hmps gapfilled to correct place
...
sdlhmp_pred$flag_1[grepl("multi", sdlhmp_pred$method) & sdlhmp_pred$airtemp_avg_pval <= 0.05] <- "B"
sdlhmp_pred$flag_1[grepl("multi", sdlhmp_pred$method) & sdlhmp_pred$airtemp_avg_pval > 0.05] <- "C"
sdlhmp_pred$flag_1[grepl("moving", sdlhmp_pred$method) & sdlhmp_pred$airtemp_avg_pval <= 0.05] <- "D"
sdlhmp_pred$flag_1[grepl("moving", sdlhmp_pred$method) & sdlhmp_pred$airtemp_avg_pval > 0.05] <- "E"
...
write_csv(edi_dat, paste0(datpath, "homogenize/nwt_sdl_homogenized_temperature_draft_2023.csv"))
saveRDS(edi_dat, paste0(datpath, "homogenize/nwt_sdl_homogenized_temperature_draft_2023.rds"))
