...
# Infilling!
################################################################################
# TODO can make this more modular and should add functionality to sip if _missing == 0

# -- C1 Chart 2019 - 2022

c1_chart_missing <- idInfillDates(alldats, "c1_chart", 2022) 
...
ggplot(c1_chart_predicted %>% dplyr::filter(yr > 2021)) +
  geom_line(aes(date, airtemp_avg))+
  geom_point(data = c1_chart_predicted %>% dplyr::filter(yr > 2021 & airtemp_avg_method != 'raw'), 
             aes(date, airtemp_avg, color = 'predicted'))+
  geom_line(aes(date, airtemp_max, color = 'max'), alpha = 0.8)+
  geom_point(data = c1_chart_predicted %>% dplyr::filter(yr > 2021 & airtemp_max_method != 'raw'), 
             aes(date, airtemp_max, color = 'predicted'), alpha = 0.9)+
  geom_line(aes(date, airtemp_min, color = 'min'), alpha = 0.5)+
  geom_point(data = c1_chart_predicted %>% dplyr::filter(yr > 2021 & airtemp_min_method != 'raw'), 
             aes(date, airtemp_min, color = 'predicted'), alpha = 0.8)+
...
ggplot(c1_chart_predicted |> dplyr::filter(yr > 2021))+
  ...
  xlim(c(lubridate::date("2022-01-01"), lubridate::date("2023-12-31")))+
...
d1_chart_missing <- idInfillDates(alldats, "d1_chart", 2023) 
...
ggplot(d1_chart_predicted %>% dplyr::filter(yr > 2021)) +
  geom_line(aes(date, airtemp_avg))+
  geom_point(data = d1_chart_predicted %>% dplyr::filter(yr > 2021 & airtemp_avg_method != 'raw'), 
             aes(date, airtemp_avg, color = 'predicted'))+
...
# -- sdl hmp 1 ----
sdl1000_hmp_1_missing <- idInfillDates(alldats, "sdl_cr1000_hmp_1", 2021)
...
# how does it look?
ggplot(sdl1000_hmp_2_predicted |> dplyr::filter(yr == 2023)) +
  #geom_vline(aes(xintercept = as.Date("1989-01-01"))) +
  geom_line(aes(date, airtemp_avg), col = "green", alpha = 0.3) +
  geom_line(aes(date, airtemp_max), col = "purple", alpha = 0.3) +
  geom_point(data = subset(sdl1000_hmp_2_predicted |> dplyr::filter(yr == 2023), airtemp_max_method != "raw"), aes(date, airtemp_max), col = "red", alpha = 0.5) +
  geom_line(aes(date, airtemp_min), col = "blue", alpha = 0.3) +
  geom_point(data = subset(sdl1000_hmp_2_predicted |> dplyr::filter(yr == 2023), airtemp_min_method != "raw"), aes(date, airtemp_min), col = "dodgerblue", alpha = 0.5) +
  geom_point(data = subset(sdl1000_hmp_2_predicted |> dplyr::filter(yr == 2023), flagmin), aes(date, airtemp_min), size = 2, col = "blue", alpha = 0.8) +
  geom_point(data = subset(sdl1000_hmp_2_predicted |> dplyr::filter(yr == 2023), flagmin), aes(date, airtemp_avg), size = 2, col = "forestgreen", alpha = 0.8) +
  geom_point(data = subset(sdl1000_hmp_2_predicted |> dplyr::filter(yr == 2023), flagmin), aes(date, airtemp_max), size = 2, col = "chocolate", alpha = 0.8) # only 1 dat
...
# -- WRITE OUT
write.csv(alldats, file = paste0(datpath,"infill/alldats.csv"),
          row.names = FALSE)
write.csv(c1_chart_predicted, file = paste0(datpath,"infill/c1_chart_infilled_v1.csv"),
          row.names = FALSE)
write.csv(d1_chart_predicted, file = paste0(datpath,"infill/d1_chart_infilled_v1.csv"),
          row.names = FALSE)
write.csv(sdl_hmps_predicted, file = paste0(datpath,"infill/sdl_hmps_infilled_v1.csv"),
          row.names = FALSE)

saveRDS(sdl_hmps_predicted, paste0(datpath,"infill/sdlhmp_infilled_2023.rds"))
