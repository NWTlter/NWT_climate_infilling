...
if(generate_plots){
  ggplot(c1_chart_tmp, aes(date, measurement))+
    ...
    geom_vline(aes(xintercept = lubridate::date("2024-01-01")), color = 'red')+
    geom_vline(aes(xintercept = lubridate::date("2025-01-01")), color = 'red')
  ...
  ggplot(nwtlog %>% subset(local_site == 'c1'), aes(date, measurement))+
    ...
    geom_vline(aes(xintercept = lubridate::date("2024-01-01")), color = 'red')+
    geom_vline(aes(xintercept = lubridate::date("2025-01-01")), color = 'red')+
    ggtitle('C1 temp data with 2024 marked')
}
...
charttemp_out <- charttemp %>%
  mutate(raw = measurement,
         ...
         measurement = ifelse(is.na(flag) | flag == 'n' | flag == "q", measurement, NA)) %>%
...
# # Taking a closer look at the sdl HMPs in 2024
if(generate_plots){
...
nwtlog_out <- nwtlog_out %>% mutate(
  measurement = ifelse(flag != 'n' | is.na(flag), NA, measurement)
)
...
lttrepo <- "~/FilingCabinet/NWTLTER/long-term-trends/"
...
# remove aspirated from this dataset to avoid dups. Included in hmpgl4d1cr_qc
logtemp_qc <- logtemp_qc |> filter(!grepl("_as", local_site))
# --- Stacking Data ------
# put it all together:
...
allnwtlog_qc$flag_ws <- apply(allnwtlog_qc[grep("ws_sc", names(allnwtlog_qc))], MARGIN = 1, function(x) sum(abs(x) >= 4) == 2)
allnwtlog_qc$flag_xs <- apply(allnwtlog_qc[grep("xs_sc", names(allnwtlog_qc))], MARGIN = 1, function(x) sum(abs(x) >= 4) == 2)
