...
# loggerppt <- get_tidydat("loggerPPT", rdsfiles_qc, "ppt") |> 
#   filter(station_name == "NPN")# Get the ETI Gauge 
...
sdlppt_out_complete$Flag.1[grepl("^H$", sdlppt_out_complete$method)] <- "A" # data is from target station££££££££dsxklweel
...
#   dplyr::mutate(
#     measurement = ifelse(
#       local_site == "d1" & date == lubridate::date("2011-12-29"),
#       NA,
#       measurement
#     )
#   )
...
d1ppt_out <- subset(chartppt, local_site == "d1" & yr >= 1980) %>%
  ...
  mutate(ppt_tot = ifelse(qdays > 1, raw, measurement)) |> 
  mutate(
    qdays = ifelse(date == lubridate::date("2012-01-16"),
      18, qdays),
    raw_qdays = ifelse(date == lubridate::date("2012-01-16"),
      18, raw_qdays)#*BUG fixed 
  )
...
d1ppt_out <- left_join(d1ppt_out, d1chosen) %>% 
  mutate(exp.ppt = exp(infill))
d1ppt_out <- backfill_ppt(d1ppt_out) #*BUG fixed 
...
# calculate exponentiated backfilled values + infill single missing days
d1ppt_out_complete <- backfill_ppt(d1ppt_out_complete)#*BUG fixed 
# fix flagging
