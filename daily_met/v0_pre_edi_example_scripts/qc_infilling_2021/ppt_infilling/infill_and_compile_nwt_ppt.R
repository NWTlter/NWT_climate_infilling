...
# infill -- breaks at 2019-04-23 because qdays of 2 indicated by ppt value is NA (nothing to backfill)
# > EDI metadata say: 2019-04-23 *Pen off chart, Total ppt for 4/19-4/21 = .2 inches (5.08 mm, which is there on 4/21)
# > for now assign NA to qdays on 2019-04-23 and use infill instead
d1_2011[which(!is.na(d1_2011$qdays) & is.na(d1_2011$ppt_tot)),] # correct row
d1_2011$qdays[which(!is.na(d1_2011$qdays) & is.na(d1_2011$ppt_tot))] <- NA
...
mutate(d1_2011.2,
  ...
  ggplot(aes(doy, backfill, col = is.na(equation))) +
  geom_point(alpha = 0.5) +
  facet_wrap(~yr) # seems reasonable.. there is a high value summer 2012, but maybe that event actually occurred if predicted by other stations?
...
# -- REALITY CHECK -----
# check that compiled data 2011-through end of 2018 (last update to EDI) matches EDI infilled versions (it should)
# if so, procedure for 2019-2020 good
d1k_edi <- getTabular(186) # goes through 2020..
c1k_edi <- getTabular(184) # goes through 2020 (but there were no missing values at C1 in 2019 and 2020, so no problem)

d1test <- subset(d1_compiled2, select = c(LTER_site, local_site, year, date, precipitation.mm:raw_qdays)) %>%
  cbind(edi_precip = d1k_edi$precip, edi_flag1 = d1k_edi$flag_1, edi_nobs = d1k_edi$num_obs_in_regression_equation, 
        edi_source = d1k_edi$source_station, edi_pval = d1k_edi$pvalue, edi_r2 = d1k_edi$rsquared) %>%
  mutate(diffppt = round(precipitation.mm ,2)- round(edi_precip,2))

d1_mismatch <- subset(d1test, year > 2010 & diffppt != 0) %>%
  dplyr::select(-c(LTER_site, regression.equation, raw_ppt_tot, raw_qdays)) %>%
  filter(abs(diffppt)>=1) # 
# pause 2021-11-26: about 57 predicted/infilled vals for just 2011-2018 differ from edi dataset (by as much as -27 to +10 mm..)
# > I assume because more data available = site hierarchy + regression performance => different best choice selections?
# > but look into this further later (for now, thru 2020 datasets are on EDI for renewal folks)
# > once sorted out, add SDL infilling to this script so SLD ppt treated in same way as C1 and D1

# how many does this apply to for c1?
c1test <- subset(c1_compiled2, select = c(LTER_site, local_site, year, date, precipitation.mm:raw_qdays)) %>%
  cbind(edi_precip = c1k_edi$precip, edi_flag1 = c1k_edi$flag_1, edi_nobs = c1k_edi$num_obs_in_regression_equation, 
        edi_source = c1k_edi$source_station, edi_pval = c1k_edi$pvalue, edi_r2 = c1k_edi$rsquared) %>%
  mutate(diffppt = round(precipitation.mm ,2)- round(edi_precip,2))

c1_mismatch <- subset(c1test, year > 2010 & diffppt != 0) %>%
  dplyr::select(-c(LTER_site, regression.equation, raw_ppt_tot, raw_qdays)) %>%
  filter(abs(diffppt)>=1) # 49 rows pulled.. 
# did methods change?
with(c1_mismatch, sapply(split(edi_flag1, flag.1), function(x) summary(as.factor(x))))
# lookin closer, it seems like it is due to spatial and methods hierarchy.. is going w method 1 if pval < 0.05 (even if method 2 in current edi dataset pval is way smaller)
# and for method 2, new value has a better pval, or is closer site, or same site but with more obs now (even if pval is bigger (but still under 0.05))
# talk to Sarah about how to move forward infilling (seems like the rule should be then to cut off at current year if we're infilling each year)

# can apply code to SDL tho.. just need to first figure out shift in 1990s (or if will infill for each period) and how to treat winter correction
# > my inclination is since tim kittel used uncorrect sdl in infilling, do the same and then correct after infilling done
# do bring in other stations since just in case can't infill 1981 foward using only NWT and nearby snotel stations




# -- WRITE OUT INFILLED DATASETS ----
# format dataset so matches what's on EDI (how others wrote scripts for renewal analyses)
# c1
#write_csv(c1_compiled2, "c1_d1_sdl_clim/homogenize_climdat/data/c1_dailyppt_infilled_1952-ongoing.csv")

# d1
#write_csv(d1_compiled2, "c1_d1_sdl_clim/homogenize_climdat/data/d1_dailyppt_infilled_1952-ongoing.csv")