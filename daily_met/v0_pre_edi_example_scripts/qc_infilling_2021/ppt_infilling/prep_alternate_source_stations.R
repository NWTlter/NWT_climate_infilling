...
# Allenspark Lodge *very patchy data (not much available for period)
...
# Grand Lake 6 SSW * available 1981-2019, ordered from NOAA
...
rawdat <- "/Users/scarlet/Documents/nwt_lter/nwt_climate/raw_data"
...
for(i in 1:length(datfiles[grep("csv", datfiles)])){
  datlist[[i]] <- data.frame(read.csv(datfiles[i], na.strings = na_vals))
}
...
fraser2 <- datlist[[8]] # same as fraser, but from CSU climate center, american units
berthoud <- datlist[[9]] # needs metric con
ned2NNE <- datlist[[10]] # needs metric con
estes2 <- datlist[[11]] # needs metric con
estes3 <- datlist[[12]] # needs metric con
...
# Nov 2021 update:(
# noaa now delivers all requested sites in 1 file.. with flags/attributes.. keeping code above and will compare data (expect will math)
allsites_noaa <- datlist[[7]]
...
# Nov 2021 update: write
allsites_noaa_clean <- clean_dat(allsites_noaa)
